...
import com.dave.nauweather.api.ApiManager;
...
public class HourlyForecastView extends View {
	...
	private  ArrayList<HourlyForecastBean.HourlyBean> forecastList;
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < length; i++) {
			...
			if(i == 0){
				...
				String datatime = d.date.substring(11);
				datatime = datatime.substring(0,datatime.lastIndexOf("+"));
				Log.d(TAG,"onDraw datatime="+datatime);
				canvas.drawText(d.tmp + "°", x[i], y[i] - textSize + textOffset, paint);
				canvas.drawText(datatime, x[i], textSize * 7.5f + textOffset, paint);// 日期d.date.substring(5)
	...
	public void setData(HourlyForecastBean body) {
		try {
			final ArrayList<HourlyForecastBean.HourlyBean> w_hourlyForecast =
					(ArrayList<HourlyForecastBean.HourlyBean>) body.hourly;
			if (w_hourlyForecast.size() == 0) {// 有可能为空
				return;
			}
					/*if (!ApiManager.isToday(w_hourlyForecast.get(0).fxTime)) {// 不是今天的数据
						return;
					}*/
			forecastList = w_hourlyForecast;
			...
			for (int i = 0; i < forecastList.size(); i++) {
				HourlyForecastBean.HourlyBean forecast = forecastList.get(i);
				int tmp = Integer.valueOf(forecast.temp);
				if (all_max < tmp) {
					all_max = tmp;
				}
				if (all_min > tmp) {
				...
				data.tmp = tmp;
				data.date = forecast.fxTime;
				Log.d(TAG,"setData forecast.fxTime="+forecast.fxTime);
				data.wind_sc = forecast.windSpeed;
